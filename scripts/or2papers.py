...
def main(username, password, venue, download_all, download_pdfs):
    ...
    for paper in submissions:
      ...
      decision_notes = client_acl.get_notes(forum=paper.forum, invitation=venue + f'/Paper{paper_number}/-/Decision')      
      
      if len(decision_notes) > 0:
        decision_note = decision_notes[0]
        paper_title = paper.content['title']
        decision = decision_note.content['decision']
        if decision == "Accept":
          decision_by_forum[paper.forum] = decision_note
      else:
        print(f'No decision found for paper {paper_number}\n')
    
    # decision_by_forum = {
        # d.forum: d
        # for d in list(
            # openreview.tools.iterget_notes(
                # client_acl, invitation=venue + "/Paper[0-9]+/-/Decision"
            # )
        # )
    ...
    for submission in tqdm(submissions):
        ...
        author_names = submission.details["original"]["content"]["authors"]
        authors = []
        for authorsid, author_name in zip(authorsids, author_names):
            ...
            if error:
                ...
                author_data = {"name": author_name, "last_name": name.last, "first_name": name.first, "middle_name": name.middle}
                small_log.write(f"Used nameparser to parse name into {author_data}\n")
