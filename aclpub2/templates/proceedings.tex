...
% General use macros
\BLOCK{macro committee_member(member)}\VAR{member.name}\BLOCK{if member.institution is defined}, \VAR{member.institution}\BLOCK{endif}\BLOCK{endmacro}
...
\begin{titlepage}
  ...
  \ThisURCornerWallPaper{1.15}{sponsor_logos/background_lq.jpg}
  ...
  {\sffamily
    ...
    \doublespacing
    \color{white}
    {\fontsize{50}{60}\selectfont NEALT}\\
    {\large Northern European Association for Language Technology}\\
    {\LARGE NEALT Proceedings Series No. 52}

    \vspace*{75mm}

    \textbf{\textsf{\Huge  Proceedings of the 24th Nordic Conference on Computational Linguistics (NoDaLiDa)}}

    \vspace*{5cm}

     \vfill
     
    {\Large May 22 - 24, 2023}\\[0.5cm]
    {\Large Tórshavn, Faroe Islands}\\[1cm]
    {\Large Editors: Tanel Alumäe and Mark Fishel}
...
\BLOCK{macro generate_sponsor_tier(tier)}
  ...
  \nopagebreak
  \BLOCK{for logo_batched in tier.logos|batch(3)}
    \BLOCK{for logo in logo_batched}
      \begin{minipage}[c][0.28\linewidth][c]{0.28\linewidth}
...
NEALT Proceedings Series, No. 52 \\
...
\BLOCK{for talk in invited_talks}
  \begin{center}
    {\LARGE \textbf{Invited Talk: \VAR{talk.title}}\\}
    \vspace*{0.5cm}
